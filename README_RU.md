...
4.Создаем на локальном компьютере файлы SSH ключей, запуская первый локальный плейбук из папки с плейбуками 
**HOME/ВАШ_ПОЛЬЗОВАТЕЛЬ/ansible-project/**:

```ansible-playbook -i hosts.ini localhost_playbook.yml```

5.Если ключи успешно созданы, то они появятся по умолчанию в папке `~/.ssh/`

6.Копируем публичный ключ на удаленный сервер, не забывая заменить IP адрес в команде на IP своего сервера:
...
7.Вводим пароль от root пользователя при запросе в терминале, чтобы подтвердить копирование SSH ключей
8.Чтобы проверить, что ключи успешно скопированы, пробуем зайти на сервер без пароля с помощью команды:

```ssh root@111.111.111.111```

9.Запускаем плейбук настройки WireGuard на сервере:

```ansible-playbook -i hosts.ini playbook.yml```

10.Вам необходимо скачать приложение WireGuard для вашего устройства (Windows, Linux, Android, iOS etc).

https://www.wireguard.com/install/

11.Создаем конфигурационные файлы для клиентов, которые можно будет скачать и использовать в приложении WireGuard

```ansible-playbook -i hosts.ini conf_file_playbook.yml```

12.Скачиваем файлы конфигов для клиентов на локальный компьютер:

```ansible-playbook -i hosts.ini download_conf_files.yml```

13.Указываем в приложении на локальном комьютере нужный файл конфигов для нужного клиента из папки, которую мы указали в настройках:
...
14.Нажимаем подключить, проверяем работу VPN, например тут:
